...
def plot_policy(ax, env, pi):
    ...
    for key, value in pi.items():
        #plt.text(key[1], key[0], action_index[np.argmax(value)] ,ha='center', va='center', fontsize=20)
        plt.text(key[1], key[0], round(np.max(value), 0) ,ha='center', va='center', fontsize=7)
...
def main():
    ...
    reward_per_episode1 = play(env, agent_Q_learning, num_episodes=num_episodes, save_policy_interval=save_policy_interval, policy=Q_policy)
    ...
    for i, q in enumerate(Q_policy):
        ...
        _ = ax[i].set_title("Q-learning Greedy policy at {} th episode".format(i*save_policy_interval))
    plt.savefig('Q_learning_policy.pdf')
    ...
    reward_per_episode2 = play(env, agent_Double_Q_learning, num_episodes=num_episodes, save_policy_interval=save_policy_interval, policy=Double_Q_policy)
    ...
    for i, q in enumerate(Double_Q_policy):
        plot_policy(ax[i], env, q)
        _ = ax[i].set_title("Double Q-learning Greedy policy at {} th episode".format(i*save_policy_interval), fontsize=12)
    plt.savefig('double_Q_learning_policy.pdf')
