...
def build_argument_parser() -> ArgumentParser:
    ...
    parser.add_argument('--save_path', default = "./trained_model.pt")
...
def main(domain: str,
    ...
    pol_optimizer = Adam(policy.parameters(), lr=learning_rate)
    cri_optimizer = Adam(policy.critic.parameters(), lr = learning_rate)
...
def rollout(env: Environment,
    ...
    reward_batch = to_tensor(np.asarray(rewards))
    return Trajectory(observations_batch, actions_batch, returns_batch, reward_batch)
...
def train(env: Environment,
    ...
    for episode in trange(num_episodes):
        ...
        q_w = policy.critic(trajectory.observations)
        q_w_prime = q_w
        q_w_prime = torch.cat([q_w_prime, q_w_prime[-1].unsqueeze(0)], dim=0)
        q_w_prime = q_w_prime[1:]
        q_w_prime[-1] = torch.zeros(1, q_w_prime.shape[-1])
        delta = trajectory.rewards + ((gamma * q_w_prime - q_w) @ torch.ones(q_w.shape[-1], 1) ) / torch.sum(torch.ones(q_w.shape[-1], 1))
        distribution = Independent(Normal(locs, scales), 1)
        log_probs = distribution.log_prob(trajectory.actions)

        # print(distribution)
        # print(log_probs.size())
        loss = abs(torch.sum(log_probs * ( q_w @ torch.ones(q_w.shape[-1], 1) / torch.sum(q_w @ torch.ones(q_w.shape[-1], 1)))))
        
        criterion = nn.MSELoss()
        critic_loss = criterion(delta, torch.ones(delta.shape[0], delta.shape[1]) * torch.mean(delta))
